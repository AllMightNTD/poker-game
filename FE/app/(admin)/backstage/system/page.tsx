...
import { RHFTextArea } from "@/components/ui/form/RhfFields";
...
const systemBroadcastSchema = z.object({
  message: z
    .string()
    .min(1, "Please enter the notification content")
    .max(200, "Maximum 200 characters for notification content")
...
export default function AdminSystemPage() {
  ...
  const onSubmit = async (data: SystemBroadcastValues) => {
    ...
    try {
      ...
      if (res.data?.success) {
        setStatus({ type: "success", text: "Notification broadcasted successfully to all online players!" });
        reset({ message: "" });
      } else {
        setStatus({ type: "error", text: res.data?.message || "Failed to broadcast notification" });
      }
    } catch (e: any) {
      console.error(e);
      setStatus({
        type: "error",
        text: e.response?.data?.message || "An error occurred while connecting to the server"
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-100">System Administration</h1>
        <p className="text-slate-500 text-sm mt-1">Configure system settings, broadcast notifications, and manage active devices.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        {/* Column 1: Broadcast */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-6">
          <h2 className="text-lg font-medium text-slate-200">System Messages</h2>
          ...
          <div className="flex gap-4 p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-xl text-sm text-indigo-300">
            ...
            <div className="space-y-1">
              <p className="font-semibold text-slate-200">Note when broadcasting notifications:</p>
              <p className="text-slate-400 leading-relaxed">
                The message will be displayed instantly on the marquee for all players in the lobby and tables.
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1">
              <RHFTextArea
                ...
                placeholder="Enter system notification here (maximum 200 characters)..."
                rows={4}
                maxLength={200}
                error={errors.message?.message}
                disabled={isSubmitting}
              />
              <div className="text-right text-xs text-slate-600">{messageValue.length}/200 characters</div>
            </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-medium text-slate-200">Active Login Sessions</h2>
            <button
          ...
          <p className="text-slate-400 text-xs leading-relaxed">
            List of devices/browsers with active logins for all players. You can remotely revoke (log out) any active session to automatically log out that user.
          </p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-semibold text-slate-200">
                        {session.user ? `${session.user.user_name} (${session.user.email})` : "Unknown User"}
                      </p>
