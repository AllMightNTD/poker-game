...
export class SeedRecommendationTestData1781000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        let targetUserId = '';
        const existingUsers = await queryRunner.query('SELECT id FROM users LIMIT 1');
        ...
        if (existingUsers && existingUsers.length > 0) {
            ...
            await queryRunner.query(`DELETE FROM profiles WHERE user_id = ?`, [targetUserId]);
            ...
            `, [targetUserId, 'https://api.dicebear.com/7.x/avataaars/svg?seed=Target']);
        }
        ...
        const insertUser = async (id: string, email: string, name: string, username: string, city = 'Da Nang', country = 'Vietnam', edu = '[]', work = '[]') => {
        ...
        await queryRunner.query(`DELETE FROM blocks WHERE blocker_id IN (?) OR blocked_id IN (?)`, [testUserIds, testUserIds]);
        await queryRunner.query(`DELETE FROM friend_requests WHERE sender_id IN (?) OR receiver_id IN (?) OR id IN ('rec-req-incoming', 'rec-req-outgoing')`, [testUserIds, testUserIds]);
        await queryRunner.query(`DELETE FROM friends WHERE user_id IN (?) OR friend_id IN (?) OR user_id = ? OR friend_id = ?`, [testUserIds, testUserIds, targetUserId, targetUserId]);
        await queryRunner.query(`DELETE FROM refresh_tokens WHERE user_id IN (?) OR id IN ('rec-token-target', 'rec-token-cand')`, [testUserIds]);
        await queryRunner.query(`DELETE FROM comments WHERE target_id = 'rec-test-post-uuid' OR user_id IN (?)`, [testUserIds]);
        await queryRunner.query(`DELETE FROM reactions WHERE target_id = 'rec-test-post-uuid' OR user_id IN (?)`, [testUserIds]);
        await queryRunner.query(`DELETE FROM posts WHERE id = 'rec-test-post-uuid' OR user_id IN (?)`, [testUserIds]);
        await queryRunner.query(`DELETE FROM group_members WHERE group_id = 'rec-test-group-uuid' OR user_id IN (?)`, [testUserIds]);
        await queryRunner.query(`DELETE FROM \`groups\` WHERE id = 'rec-test-group-uuid'`);

        // Signal 1: Mutual Friends Candidate (Highest weight)
        await insertUser('rec-user-mutual-uuid', 'rec_mutual@example.com', 'Mutual Friend Candidate', 'rec_mutual');
        
        // Connectors (Friends of both target and mutual)
        await insertUser('rec-friend-1-uuid', 'rec_friend1@example.com', 'Connector One', 'rec_friend1');
        await insertUser('rec-friend-2-uuid', 'rec_friend2@example.com', 'Connector Two', 'rec_friend2');
        await insertUser('rec-friend-3-uuid', 'rec_friend3@example.com', 'Connector Three', 'rec_friend3');

        // Target user is friends with Connectors
        for (const fId of ['rec-friend-1-uuid', 'rec-friend-2-uuid', 'rec-friend-3-uuid']) {
            await queryRunner.query(`INSERT INTO friends (user_id, friend_id, created_at) VALUES (?, ?, NOW()), (?, ?, NOW())`, [targetUserId, fId, fId, targetUserId]);
        }
        // Mutual Candidate is also friends with Connectors (giving exactly 3 mutual friends)
        for (const fId of ['rec-friend-1-uuid', 'rec-friend-2-uuid', 'rec-friend-3-uuid']) {
            await queryRunner.query(`INSERT INTO friends (user_id, friend_id, created_at) VALUES ('rec-user-mutual-uuid', ?, NOW()), (?, 'rec-user-mutual-uuid', NOW())`, [fId, fId]);
        }
        ...
        await insertUser('rec-user-interaction-uuid', 'rec_interaction@example.com', 'Interaction Candidate', 'rec_interaction');
        ...
        await insertUser('rec-user-network-uuid', 'rec_network@example.com', 'Network Candidate', 'rec_network');
        ...
        await insertUser('rec-user-already-friend-uuid', 'rec_friend@example.com', 'Already Friend Excluded', 'rec_friend');
        await queryRunner.query(`INSERT INTO friends (user_id, friend_id, created_at) VALUES (?, 'rec-user-already-friend-uuid', NOW()), ('rec-user-already-friend-uuid', ?, NOW())`, [targetUserId, targetUserId]);

        // Exclusion 2: Pending incoming request
        await insertUser('rec-user-pending-inc-uuid', 'rec_pending_inc@example.com', 'Pending Incoming Excluded', 'rec_pending_inc');
        await queryRunner.query(`
        ...
        await insertUser('rec-user-pending-out-uuid', 'rec_pending_out@example.com', 'Pending Outgoing Excluded', 'rec_pending_out');
        await queryRunner.query(`
            INSERT INTO friend_requests (id, sender_id, receiver_id, status, created_at)
        ...
        await insertUser('rec-user-blocked-uuid', 'rec_blocked@example.com', 'Blocked User Excluded', 'rec_blocked');
        await queryRunner.query(`
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        // Delete friends
        await queryRunner.query(`DELETE FROM friends WHERE user_id IN (?) OR friend_id IN (?)`, [testUserIds, testUserIds]);
        ...
        // Delete profiles
        await queryRunner.query(`DELETE FROM profiles WHERE user_id IN (?)`, [testUserIds]);
