...
export class TypeOrmPostRepository implements IPostRepository {
  ...
  async updatePost(userId: string, postId: string, updatePostDto: any): Promise<any> {
    const post = await this.postRepository.findOne({ where: { id: postId } });
    if (!post) {
      throw new NotFoundException('Post not found');
    }
  ...
  private async broadcastNewPostRealtime(post: any) {
    try {
      ...
      console.error('[PostRepository] Error during realtime post broadcasting:', error.message);
    }
  }
  ...
  private async broadcastPostUpdateRealtime(post: any) {
    try {
      ...
      console.error('[PostRepository] Error during realtime post update broadcasting:', error.message);
    }
  }
  ...
  private async broadcastReactionRealtime(post: Post, stats: any) {
    try {
      ...
      console.error('[PostRepository] Error during realtime reaction broadcasting:', error.message);
    }
  }
}
