  ...
  PokerTable: {
    findOne: jest.fn().mockImplementation(({ where }) => {
      if (where.id === 'allow-chat-room') {
        return Promise.resolve({ id: 'allow-chat-room', is_active: true, custom_settings: { allow_chat: true } });
      }
      if (where.id === 'no-chat-room') {
        return Promise.resolve({ id: 'no-chat-room', is_active: true, custom_settings: { allow_chat: false } });
      }
...
jest.mock('../entities/user.entity', () => ({
  User: {
    findOne: jest.fn().mockImplementation(({ where }) => {
      return Promise.resolve({ id: where.id, user_name: `user_${where.id}`, avatar_url: `avatar_${where.id}` });
    })
  },
}));
...
describe('PokerLobbyGateway - Chat Realtime & History', () => {
  ...
  it('CHAT-001: Player gửi tin nhắn -> lưu vào Redis và broadcast tức thì đến cả phòng', async () => {
    ...
    await gateway.handleChatMessage(mockClient as any, { room_id: roomId, message });
    ...
    expect(gateway.server.emit).toHaveBeenCalledWith('table:chat-message-received', expect.objectContaining({
      user_id: 'user_1',
      message: 'Hello, world!',
      seat_number: 1,
    }));
  ...
  it('CHAT-002: Không cho phép chat nếu custom_settings.allow_chat = false', async () => {
    ...
    expect(mockClient.emit).toHaveBeenCalledWith('error', { message: 'Chủ phòng đã tắt tính năng chat.' });
  ...
  it('CHAT-003: Lấy lịch sử chat (Pagination)', async () => {
    ...
    // Inject 5 messages
    for (let i = 1; i <= 5; i++) {
      await stateService.pushChatMessage(roomId, JSON.stringify({ message: `Msg ${i}`, timestamp: Date.now() + i }));
    }

    // Fetch offset 0 limit 2 (newest 2 messages)
    await gateway.handleGetChatHistory(mockClient as any, { room_id: roomId, offset: 0, limit: 2 });
    expect(mockClient.emit).toHaveBeenCalledWith('table:chat-history-loaded', expect.objectContaining({
      room_id: roomId,
      offset: 0,
      limit: 2,
      hasMore: true,
    ...
    await gateway.handleGetChatHistory(mockClient as any, { room_id: roomId, offset: 2, limit: 2 });
