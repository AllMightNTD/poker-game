...
export class AdminTransactionsService {
  async getTransactions(cursor: string | undefined, limit: number, status?: string) {
    const query = Transaction.createQueryBuilder('tx')
    ...
    const data = transactions.map(tx => ({
      ...tx,
      user: tx.user ? { id: tx.user.id, email: tx.user.email, user_name: tx.user.user_name } : null
    }));

    return buildCursorPaginationResponse(data as any, limit);
  }

  async processTransaction(adminId: string, id: string, dto: ProcessTransactionDto) {
    const transaction = await Transaction.findOne({ where: { id } });
    if (!transaction) throw new NotFoundException('Transaction not found');
    if (transaction.status !== 'PENDING') throw new BadRequestException('Transaction is already processed');
    ...
    if (dto.status === 'APPROVED' && transaction.type === 'DEPOSIT') {
      ...
      if (wallet) {
        wallet.chips_balance = (BigInt(wallet.chips_balance) + BigInt(transaction.amount)).toString();
        await wallet.save();
      }
    }
