...
export class PokerBotManager {
  ...
  async checkAndTriggerBotAction(roomId: string) {
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          const currentTableState =
            await this.gameService.stateService.getTableState(roomId);
          if (!currentTableState || currentTableState.game_stage === 'ended')
            return;
          if (
            parseInt(currentTableState.current_turn_seat || '0') !==
            currentTurnSeat
          )
            return;
          ...
          const currentBotSeat = currentSeats.find(
            (s) => s.seat_number === currentTurnSeat,
          );
          if (
            !currentBotSeat ||
            currentBotSeat.is_bot !== '1' ||
            currentBotSeat.status !== 'active'
          )
            return;

          const pocket = await this.gameService.stateService.getPlayerCards(
            roomId,
            currentBotSeat.user_id,
          );
          const community = currentTableState.community_cards
            ? currentTableState.community_cards.split(',')
            : [];

          const handStrength = PokerBotAI.getHandStrength(pocket, community);

          const dealerSeat = parseInt(currentTableState.dealer_seat || '1');
          const positionLabel = PokerBotAI.getPositionLabel(
            currentTurnSeat,
            dealerSeat,
            currentSeats,
          );
          ...
          const currentBet = parseInt(
            String(currentBotSeat.current_bet || '0'),
          );
          const highestBet = parseInt(
            currentTableState.current_highest_bet || '0',
          );
          ...
          this.gameService.logger.log(
            `Bot ${currentBotSeat.username} (Seat ${currentTurnSeat}, Pos ${positionLabel}, Strength ${handStrength.toFixed(2)}) decides: ${decision.action} with amount ${decision.amount}`,
          );
          ...
          this.gameService.logger.error(
            `Error executing bot action: ${err.message}`,
          );
        } finally {
      ...
      this.gameService.logger.error(
        `Error in checkAndTriggerBotAction: ${err.message}`,
      );
    }
  }
}
