...
export class PokerShowdownManager {
  ...
  async processShowdown(roomId: string) {
    ...
    const community1 = tableState.community_cards ? tableState.community_cards.split(',') : [];
    const community2 = isRitActive && tableState.rit_board2_cards ? tableState.rit_board2_cards.split(',') : [];
    ...
    const evaluatedPlayers1 = await Promise.all(
      activePlayers.map(async p => {
        ...
        this.gameService.logger.log(`[SHOWDOWN] Board 1 eval player seat=${p.seat_number} user=${p.user_id} pocket=${pocket.join(',')} score=${evalResult.score} hand=${evalResult.name}`);
    ...
    if (isRitActive) {
      evaluatedPlayers2 = await Promise.all(
        activePlayers.map(async p => {
          ...
          this.gameService.logger.log(`[SHOWDOWN] Board 2 eval player seat=${p.seat_number} user=${p.user_id} pocket=${pocket.join(',')} score=${evalResult.score} hand=${evalResult.name}`);
    ...
    for (const [i, pot] of pots.entries()) {
      if (pot.isUncalled) {
        ...
        if (player) {
          uncalledRefunds.set(player.user_id, (uncalledRefunds.get(player.user_id) || 0) + pot.amount);
        }
        continue;
      }
      ...
      if (isRitActive) {
        ...
        if (amount1 > 0) {
          const eligible1 = evaluatedPlayers1.filter(p => pot.eligibleSeats.includes(p.seat));
          if (eligible1.length > 0) {
            ...
            for (const winner of potWinners) {
              ...
              const potLabel = pots.length === 1 ? 'Board 1' : `Board 1 - Pot ${i}`;
              if (existing) {
        ...
        if (amount2 > 0) {
          const eligible2 = evaluatedPlayers2.filter(p => pot.eligibleSeats.includes(p.seat));
          if (eligible2.length > 0) {
            ...
            for (const winner of potWinners) {
              ...
              const finalWinAmount = winShare + extraChip;
              const existing = winnerMap.get(winner.user_id);
              const potLabel = pots.length === 1 ? 'Board 2' : `Board 2 - Pot ${i}`;
              if (existing) {
                existing.win_amount += finalWinAmount;
                if (!existing.pots) existing.pots = [];
                existing.pots.push({ label: potLabel, amount: finalWinAmount });
        ...
        for (const winner of potWinners) {
          ...
          const existing = winnerMap.get(winner.user_id);
          const potLabel = pots.length === 1 ? undefined : (i === 0 ? 'Main Pot' : `Side Pot ${i}`);
    ...
    await this.finalizeAndBroadcastHand(roomId, winnersLog, totalRakedPot, tableState, seats, uncalledRefunds);
  }
  ...
  async finalizeAndBroadcastHand(
    ...
    for (const [userId, refundAmount] of uncalledRefunds.entries()) {
      ...
      if (seat) {
        ...
        await this.gameService.syncSeatStackToDb(roomId, userId, newStack.toString());
        this.gameService.logger.log(`[REFUND] User ${userId} refunded ${refundAmount} uncalled bet.`);
      }
    }

    // 1. Khấu trừ Rake (mặc định 0 cho Home Game)
    const dbTable = await PokerTable.findOne({ where: { id: roomId } });
    const rakeRate = dbTable && dbTable.rake_rate !== undefined ? dbTable.rake_rate : 0;
    const rakeCap = dbTable && dbTable.rake_cap !== undefined ? BigInt(dbTable.rake_cap) : BigInt(0);
    ...
    let totalWinAmount = winnersLog.reduce((acc, w) => acc + w.win_amount, 0);
    if (rakeCalculated > BigInt(0) && totalWinAmount > 0) {
      ...
      for (let i = 0; i < winnersLog.length; i++) {
        ...
        if (i === winnersLog.length - 1) {
          ...
          const rakeShare = Math.floor((w.win_amount / totalWinAmount) * rakeNum);
    ...
    const allHandsForBroadcast = await Promise.all(
      seats
        ...
        .map(async (seat) => {
          ...
          const shouldMuck = !isWinner && autoMuck && dbTable?.custom_settings?.allow_muck;
          return {
    ...
    this.gameService.server.to(`table_${roomId}`).emit('table:hand-ended', {
      ...
      rit_board2_cards: tableState.is_rit_active === '1' && tableState.rit_board2_cards ? tableState.rit_board2_cards.split(',') : [],
      provably_fair: {
