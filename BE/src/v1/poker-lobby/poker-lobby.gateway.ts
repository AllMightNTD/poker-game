...
export class PokerLobbyGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  private async processShowdown(roomId: string) {
    ...
    for (const [i, pot] of pots.entries()) {
      ...
      for (const winner of potWinners) {
        const currentSeat = await this.stateService.getSeat(
          roomId,
          winner.seat,
        );
        ...
        if (!currentSeat) {
          console.error(
            `[SHOWDOWN] Seat missing: ${winner.seat}`,
          );
          continue;
        }
        ...
        const currentStack = parseInt(currentSeat.stack || '0');
        const newStack = currentStack + finalWinAmount;
        ...
        try {
          ...
          console.error(
            `[SHOWDOWN] DB sync error user=${winner.user_id}`,
            err,
          );
        }
    ...
    const totalPotAmount = parseInt(
      tableState.total_pot || '0',
    );

    const rakeRate = dbTable ? dbTable.rake_rate : 5.0;
    const rakeCap = dbTable
      ? BigInt(dbTable.rake_cap)
      : BigInt(0);

    let rakeCalculated = BigInt(
      Math.floor((totalPotAmount * rakeRate) / 100),
    );
    ...
    console.log(`[SHOWDOWN] rake`, {
      ...
      rakeRate,
      rakeCap: rakeCap.toString(),
      rakeCalculated: rakeCalculated.toString(),
    });

    console.log(`[SHOWDOWN] END roomId=${roomId}`);
    await this.stateService.setTableState(roomId, {
      game_stage: 'ended',
      total_pot: '0',
      current_highest_bet: '0',
  ...
  // --- Các hàm internal/helper khác của Class giữ nguyên ---
